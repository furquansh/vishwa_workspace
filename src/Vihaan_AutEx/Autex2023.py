...
class autonomous():
    ...
    def interpret_Speed(self, jack, distance =15):
        ...
        elif (jack == 2 and 10<distance<=200) :
    ...
    #     self.distance = self.depth_image[int((self.midpoint[0])[0]), int((self.midpoint[1])[1])]
            
    def calc_dist(self, knownWidth, focalLength, perWidth):
    ...
    def image_callback1(self, image_data):
        ...
        if len(left_signs)==4:
            ...
            # self.midpoint = [(x1+(x1+w1))/2, (y1+(y1+h1)/2)]
            # print(self.midpoint)
            print("left arrow being detected")
            self.distance = self.calc_dist(30,610,w1[0])-1
            print(self.distance)
            # distance_left_arrow = self.depth_image[midpoint_left[0], midpoint_left[1]]
            # distance_left_arrow = distance_left_arrow/10
            if(self.distance):
                self.interpret_Speed(2, self.distance)

        elif len(right_signs)==4:
            x2, y2, w2, h2 = right_signs
            # self.midpoint = [((x2+(x2+w2))/2), (y2+(y2+h2)/2)]
            # print(self.midpoint)
            print("right arrow being detected ")
            self.distance = self.calc_dist(30,610,w2[0])-1
            print(self.distance)
            if(self.distance):
                self.interpret_Speed(3, self.distance)
        else :
